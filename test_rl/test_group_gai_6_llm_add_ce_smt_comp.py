...
import numpy as np
from env_gai_6_llm_add_ce import ConstraintSimplificationEnv_test
...
def test_group():
    info_name = 'info_dict_gai_6_normal_1021_smt_comp.txt'
        ...
        if list1[0] == "sat" or list1[0] == "unknown":
            if list1[1] > 300:
                ...
                if file_path not in info_dict.keys():
                    # 跳过无法处理的文件
                    if 'toroidal_queen47-1.smt2' in file_path:
                        continue
