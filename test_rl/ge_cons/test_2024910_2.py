...
# 创建求解器
    ...
    const_0 = BitVecVal(0, 32)
    const_1 = BitVecVal(1, 32)
    ...
    # 增加一些位运算和非线性组合
    solver.add(((x * y * z) % (x + const_1)) * ((x ^ y) + (z >> const_1)) > const_5000)
    ...
    solver.add(((x * y * z) % (x * y + const_1)) * ((z ^ (x + y)) % const_97) == const_0)

    # 复杂表达式
    complex_expr = ((((x * const_6) ^ (x >> const_2_bit)) & const_FF) * ((x | const_3) % const_73) + (
                (x & const_1F) << const_3_bit)) % const_71 / const_2
