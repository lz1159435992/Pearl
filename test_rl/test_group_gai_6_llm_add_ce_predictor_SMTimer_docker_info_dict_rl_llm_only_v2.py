...
class LLM_no_rl:
        ...
        for chunk in chunks:
            chat_completion = client.chat.completions.create(
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": chunk + """This is the variable values from the previous failed SAT solving attempt and SMT 
    ...
    def process_text_python(self,text_ce, text_smt):
        ...
        user_message = {
            "role": "user",
            "content": text + f"""This is the variable values from the previous failed SAT solving attempt and SMT 
                                text given to you in segments; analyze it. To speed up the solution and obtain a SAT result, and identify the 
    ...
    def step(self):
        ...
        #     print(var_set)
        if len(self.counterexamples_list)==0 or len(self.counterexamples_list[-1]) != 0:
...
def extract_variables_from_smt2_content(content):
    ...
    return variables
@timeout_decorator.timeout(60*20) # 20分钟
def execute_smt(file_path, smtlib_str, var_dict, constant_list, assertions, result_list):
    model = SimpleClassifier()
    model_path = 'bert_predictor_mask_best.pth'
    ...
    model_time.eval()
    env = LLM_no_rl(CodeEmbedder_normalize(), assertions, model, model_time, smtlib_str, file_path, var_dict, constant_list)
                ...
                if file_path not in info_dict.keys():
                    ...
                    if file_path not in info_dict.keys():
                        ...
                        try:
                            ...
                            # 更改了预测器
                            result_list = execute_smt(file_path, smtlib_str, var_dict, constant_list, assertions, result_list)
                            # model = SimpleClassifier()
