...
import numpy as np
from env_gai_6_llm_add_ce_predictor_save_docker import ConstraintSimplificationEnv_test
...
# def online_for_break(agent, env, number_of_episodes, print_every_x_episodes, record_period):
#     try:
...
#         print("Time out! Bye!")
def test_group():
    info_name = 'info_dict_gai_6_normal_1109_pre_SMTimer_save_docker_llama_3.1:70b_1200s.txt'
    ...
    save_flag = False
    agent_path = 'agent_save_1109.pkl'
                ...
                if file_path not in info_dict.keys():
                    ...
                    if file_path not in info_dict.keys():
                        ...
                        try:
                            ...
                            # thread = threading.Thread(target=online_for_break,
                            #                           kwargs={
                            #                               'agent': agent,
                            #                               'env': env,
                            #                               'number_of_episodes': number_of_episodes,
                            ...
                            # thread = threading.Thread(target=online_learning,
                            #                           kwargs={
                            #                               'agent': agent,
                            #                               'env': env,
                            #                               'number_of_episodes': number_of_episodes,
                        ...
                        # plt.legend()
                        # plt.savefig(f'fig/{key.split("/")[-1]}.png')
                        # plt.show()
