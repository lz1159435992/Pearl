...
def get_values_nju(db_path, table_name):
    ...
    value_dict_2 = {}
    value_dict_1 = fetch_data_as_dict(db_path, table_name)
    # sampled_items = random.sample(value_dict_1.items(), 10000)
...
#     print('failed', e)
# nju
dict_value = get_values_nju('/home/user/Pearl/test_rl/test_script/value_dictionary.db',
                            'value_dictionary')
...
class ConstraintSimplificationEnv_test(Environment):
    ...
    def __init__(self, embedder, z3ast, num_variables, smtlib_str, file_path):
        self.step_count = 0
    ...
    def reset(self, seed=None):
        ...
        if self.num_variables == 50:
            self.actions_v = self.strings_to_onehot()
        else:
    ...
    @staticmethod
    def strings_to_onehot(string_list=None, n=50):
        ...
        # Create a list to store the one-hot tensors
        one_hot_tensors = []
        for string in string_list:
            # Create a tensor filled with zeros
            one_hot_vector = torch.zeros(n, dtype=torch.float32)
            # Encode each character in the string
            one_hot_vector[str_to_index[string]] = 1.0
            one_hot_vector.to(device)  # Move the generated tensor to the device
            one_hot_tensors.append(one_hot_vector)

        # Pad with zero tensors if the string_list length is less than n
        if len(string_list) < n:
            for i in range(len(string_list), n):
                one_hot_tensors.append(torch.zeros(n, dtype=torch.float32))
