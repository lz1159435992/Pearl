...
class LLM_no_rl:
    ...
    def process_text_python(self,text_ce, text_smt):
        ...
        5.Select one set of variables and their corresponding specific values from the SMT file so that the SMT file can be solved as satisfiable (SAT). One set of variables can include all the variables in the constraints, or it can contain only some of the variables.Variables need to be selected from the following variables：
        """ + var_str + """\n 
        Your output should conform to the strict JSON format, with an example as follows: \n```json\n [ {<variable1>: <value1>, <variable2>: <value2>,...,}]\n ``` \n <value> should confrom the general number format, like <10>."""
        }
        user_message = {
            "role": "user",
            "content": text + f"""This is the The variable values from the previous failed SAT solving attempt and SMT 
                                ...
                                can be solved as satisfiable (SAT). One set of variables can include all the variables in the constraints, or it can contain only some of the variables.Variables need to be selected from the following variables：
        """ + var_str + """\n 
        The variables and their corresponding specific values should conform to the JSON format."""
        }
    ...
    def process_action(self, action_v1, action_n1, reward):
        ...
        if action_v1 not in self.variables or int(action_n1)< self.var_range_dict[action_v1][0][0] or int(action_n1) > self.var_range_dict[action_v1][0][1]:
            reward['finish'] = True
            return reward
...
if __name__ == '__main__':
    info_name = 'info_dict_normal_1103_llm_no_rl_direct_solve_docker_llama3.1:70b_1set.txt'
