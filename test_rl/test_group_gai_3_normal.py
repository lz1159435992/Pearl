...
def test_group():
    if not os.path.exists('info_dict_gai_3_normal64.txt'):
        # 文件不存在时，创建文件
        info_dict = {}
        with open('info_dict_gai_3_normal64.txt', 'w') as file:
            json.dump(info_dict, file, indent=4)
        print(f"文件 info_dict_gai_3_normal64.txt 已创建。")
    else:
        ...
        if list1[0] == "sat":
            if list1[1] > 20:
                ...
                if file_path not in info_dict.keys():
                    ...
                    model_time.eval()
                    env = ConstraintSimplificationEnv_test(embedder, assertions, model, model_time,smtlib_str, file_path,var_dict, constant_list)
