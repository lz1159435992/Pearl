...
import numpy as np
from env_gai_6_llm_add_ce_predictor_save_local import ConstraintSimplificationEnv_test
...
class LLM_no_rl:
    ...
    def process_text(text_ce, text_smt):
    ...
    5.Select 10 sets of variables and their corresponding specific values from the SMT file so that the SMT file can be solved as satisfiable (SAT). One set of variables can include all the variables in the constraints, or it can contain only some of the variables.Your output should conform to the JSON format, with an example as follows: \n [ {<variable1>: <value1>, <variable2>: <value2>,...},{<variable1>: <value3>, <variable3>: <value4>,...} ...]"""
        }
    ...
    def process_action(self, action_v1, action_n1, reward):
        ...
        variable_pred_1 = self.variables[int(action_v1.item())]
        if int(action_n1.item()) == 0:
            if self.constant_list:
                selected_int_1 = int(random.choice(self.constant_list))
                print(selected_int_1)
                print(str(selected_int_1))
        else:
            selected_int_1 = random.randint(self.var_range_dict_n[variable_pred_1][action_n1 - 1][0],
                                            self.var_range_dict_n[variable_pred_1][action_n1 - 1][1])
        ...
        if variable_pred_1 not in self.used_variables:
            self.used_variables.append(variable_pred_1)
            related_assertions = self.v_related_assertions[variable_pred_1]
            count = 0
            if len(related_assertions) > 0:
                ...
                for a in related_assertions:
                    ...
                    new_smtlib_str = smtlib_str_before + self.new_constraint_dict[variable_pred_1] + smtlib_str_after
                    solver_related = Solver()
                    assertions = parse_smt2_string(new_smtlib_str)
                    for a in assertions:
                        solver_related.add(a)
            ...
            for a in assertions:
                solver.add(a)
            assertions_list = [a for a in assertions]
            indexes = random.sample(range(len(assertions_list)), int(len(assertions) * 0.5))
            res = [assertions_list[i] for i in sorted(indexes)]
...
The provided variable assignments are non-zero and simple, allowing the SMT solver to find satisfiable results more easily. These assignments avoid the failed combinations from the counterexamples and should meet the constraints in the SMT-LIB format."""
        ...
        if self.step_count > 10000:
            self.finish = True
        temp_observation = self.state.clone().detach()
        self.reset()
        return ActionResult(
...
if __name__ == '__main__':
    info_name = 'info_dict_gai_6_normal_1025_pre_SMTimer_save_local.txt'
