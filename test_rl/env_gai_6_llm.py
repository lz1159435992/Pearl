        ...
        for chunk in chunks:
            chat_completion = client.chat.completions.create(
                messages=[
                    {
                        "role": "user",
                        "content": chunk + f'This is the SMT text given to you in segments; analyze it. To speed up the solution and obtain a SAT result, provide a specific number that {variable_pred} should be assigned to. Output only the number, nothing else.'
                    },
                ],
                model="gpt-3.5-turbo",
                max_tokens = 10,
        ...
        try:
            ...
            for index, value in enumerate(self.counterexamples_list[-1]):
                if value[0] == variable_pred:
                    var_flag = index
            # 如果选择了第一个数，随机选择一个值
            selected_int = self.process_text(self.smtlib_str,variable_pred)
            print('llm挑选的具体值')
            print(selected_int)
