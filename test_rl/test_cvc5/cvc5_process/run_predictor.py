...
class ConstraintSimplificationEnv_test(Environment):
    ...
    def calculate_reward(self, solver_result):
        ...
        if finish:
            ...
            performance += 1
            self.finish = True
            self.solve_time = self.total_solve_time
        else:
...
def process_single_file(file_path, list1, info_dict, args):
    ...
    try:
        ...
        try:
            ...
            total_solve_time = env.solve_time if hasattr(env, 'solve_time') else 0
        except MyException:
            logger.info(f'执行超时: {file_path}')
            total_solve_time = args.timeout

        signal.alarm(0)
        end_time = time.time()
        total_execution_time = end_time - start_time
        
        # 更新结果列表，添加总执行时间和求解器时间
