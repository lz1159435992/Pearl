...
def solve_with_z3_and_record(file_path, info_dict, args):
    """
    直接使用Z3求解器求解并记录结果
    """
    logger, _ = setup_logger(batch_id=args.batch_id)
    logger.info(f"直接使用Z3求解: {file_path}")
    ...
    try:
        ...
        solver_result = solver.solve(smtlib_str, timeout=args.timeout)

        result_list = [
            solver_result.result,          # 求解结果 (sat/unsat/unknown)
            solver_result.solve_time,      # 实际求解时间
            "N/A",                         # 原始内存使用 (不适用)
            solver_result.solve_time,      # 总执行时间 (同求解时间)
            solver_result.solve_time,      # 总求解时间
            0,                             # 最终成功求解时间 (因为不是RL)
            0,                             # LLM总时间 (未使用)
            'direct_solve',                # 求解状态
            [],                            # 最终赋值序列 (不适用)
        ...
        logger.info(f"直接求解完成: {file_path}, 结果: {solver_result.result}, 时间: {solver_result.solve_time:.3f}秒")
    except Exception as e:
        logger.error(f"直接求解文件 {file_path} 时出现异常: {str(e)}")
        traceback.print_exc()
...
def run_predictor(args):
    ...
    logger.info("=" * 80)
    logger.info(f"SMT约束求解预测器启动 (预测驱动模式) - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"使用求解器: {args.solver}, LLM模型: {args.llm_model}, LLM服务器: {args.llm_host}")
    logger.info(f"随机抽样数量: {args.num_random_samples}, 预测时间阈值: {args.prediction_threshold}")
    logger.info("=" * 80)
    ...
    # --- NEW LOGIC: Randomly sample N files ---
    all_files = [key for key in rl_dict.keys() if key not in info_dict]
    if len(all_files) == 0:
        logger.info("所有文件已处理完毕，程序退出。")
        return
        
    num_to_sample = min(args.num_random_samples, len(all_files))
    if num_to_sample == 0:
        logger.info(f"没有需要处理的新文件，或指定的抽样数量为0。程序退出。")
        return
        
    selected_files = random.sample(all_files, num_to_sample)
    logger.info(f"从 {len(all_files)} 个未处理文件中随机抽取 {num_to_sample} 个进行处理。")
    ...
    try:
        ...
        logger.info("时间预测器模型加载成功。")
    except Exception as e:
        logger.error(f"加载时间预测器模型失败: {e}")
        return
    ...
    for key in selected_files:
        file_count += 1
        logger.info("=" * 80)
        logger.info(f'开始处理文件 {file_count}/{num_to_sample}: {key}')
        ...
        try:
            ...
            smtlib_str = dict_obj['smt_script'] if 'smt-comp' in key else dict_obj['script']
            ...
            with torch.no_grad():
                output_time = time_predictor(embedding)
                _, predicted_time_class = torch.max(output_time, 1)
            
            predicted_class = predicted_time_class.item()
            logger.info(f"文件 {key} 的预测求解时间类别为: {predicted_class}")

            # Dynamic decision based on prediction
            if predicted_class > args.prediction_threshold:
                logger.info(f"预测时间类别 ({predicted_class}) > 阈值 ({args.prediction_threshold})。使用RL+LLM方法求解。")
                process_single_file_with_timeout(key, ["", 0, ""], info_dict, args) # list1 is now a placeholder
            else:
                logger.info(f"预测时间类别 ({predicted_class}) <= 阈值 ({args.prediction_threshold})。使用Z3直接求解。")
                solve_with_z3_and_record(key, info_dict, args)

        except Exception as e:
            logger.error(f"处理文件 {key} 时出现顶层异常: {str(e)}")
            traceback.print_exc()
        
        # Save info_dict after each file to prevent data loss
        try:
            with open(args.info_dict_path, 'w') as file:
                json.dump(info_dict, file, indent=4)
        except Exception as e:
    ...
    logger.info("\n" + "=" * 80)
    logger.info(f"预测驱动的处理流程完成 - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    parser = argparse.ArgumentParser(description='运行SMT约束求解预测器 (预测驱动模式)')
    
    # --- NEW ARGUMENTS ---
    parser.add_argument('--num_random_samples', type=int, default=10,
                        help='随机抽取进行处理的文件数量')
    parser.add_argument('--prediction_threshold', type=int, default=3,
                        help='预测求解时间的类别阈值。大于此阈值的问题将使用RL+LLM求解 (范围0-7)')
    ...
    parser.add_argument('--eight_class_model_path', type=str,
                        ...
                        help='八分类模型路径')
    # time_threshold is no longer needed
