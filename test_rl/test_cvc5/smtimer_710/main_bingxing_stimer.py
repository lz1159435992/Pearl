...
def get_Z3_result():
    setup_logger()
    info_name = 'smtimer-533-result.txt'
...
def get_MathSAT_result():
    ...
    timeout = 1200  # 设置超时时间为1200秒
    json_path = f"{solver_name.lower()}_smtimer_results_rl.json"
    ...
    # 3. 并发求解未求解的文件
    if unsolved_smtlib:
        results = concurrent_solve(solver, unsolved_smtlib, timeout=timeout, max_workers=8)
        for i, result in enumerate(results):
            model_dict = parse_model_to_dict(result.model)
            result_dict[unsolved_paths[i]] = [
                result.result,
