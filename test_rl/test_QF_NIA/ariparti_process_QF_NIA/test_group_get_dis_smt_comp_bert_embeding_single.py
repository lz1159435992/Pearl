"""
QF_NIA数据处理和Embedding生成模块（AriParti 版本）
与 cvc5 版本保持完全一致的处理流程，仅更换求解结果数据源为 AriParti 的并行输出JSON。
...
import project_bootstrap  # 导入即生效
...
def load_ariparti_results(result_json_path):
    """
    读取 AriParti 并行批处理输出（JSON），并转换为 embedding 生成脚本所需的简化格式：
    { file_path: [result, solve_time] }
    """
    with open(result_json_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    out = {}

    # Format A (recommended in this repo):
    # { "/abs/path.smt2": ["sat|unsat|unknown|timeout", solve_time, timeout, extra] }
    if isinstance(data, dict) and data and 'results' not in data:
        for file_path, entry in data.items():
            if not isinstance(entry, list) or len(entry) < 2:
                continue
            status = (entry[0] or 'unknown').lower() if isinstance(entry[0], str) else 'unknown'
            try:
                time_val = float(entry[1])
            except Exception:
                time_val = 0.0
            out[file_path] = [status, time_val]
        return out

    # Format B (AriParti_sync/scripts style):
    # {"metadata": {...}, "results": { file: {"result":..., "solve_time":..., "total_time":...} } }
    results = data.get('results', {}) if isinstance(data, dict) else {}
    for file_path, entry in results.items():
        if not isinstance(entry, dict):
            continue
        status = (entry.get('result', 'unknown') or 'unknown').lower()
        # 优先使用 solve_time，若缺失则回退 total_time
        time_val = entry.get('solve_time', 0)
        if not isinstance(time_val, (int, float)):
            try:
                ...
                time_val = 0
        if time_val <= 0:
            t2 = entry.get('total_time', 0)
            try:
                time_val = float(t2)
            except Exception:
                pass
        out[file_path] = [status, float(time_val)]
    return out


def test_group_bert_normalize_1by1_smt_name_2_QF_NIA():
    """
    处理QF_NIA问题集并生成embedding（AriParti 结果驱动）
    ...
    # 读取 AriParti 求解结果 JSON（从 config.data.source 读取）
    ariparti_result_json = config.get('data', {}).get('source')
    if not ariparti_result_json or not os.path.exists(ariparti_result_json):
        raise FileNotFoundError(f'AriParti baseline results not found: {ariparti_result_json}')
    result_dict = load_ariparti_results(ariparti_result_json)
...
if __name__ == '__main__':
    setup_logger()
    logger.add('embedding_generation_ariparti.log')
    # 生成embeddings
