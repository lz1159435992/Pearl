...
def analyze_results(result_file):
    ...
    for file_path, result_list in results.items():
        # result_list格式: [原始状态, 原始时间, 原始超时, 执行时间, 状态, [求解时间], [反例], [反例列表]]
        if len(result_list) < 4:
            continue
        
        original_time = result_list[1]  # 原始求解时间
        execution_time = result_list[3]  # 执行时间
        ...
        if status == 'succeed':
            ...
            if len(result_list) > 5:
                solve_time = result_list[5]
                stats['solve_times'].append(solve_time)
                
                # 计算加速比
                if original_time > 0:
                    speedup = original_time / solve_time
    ...
    if stats['solve_times']:
        stats['avg_solve_time'] = np.mean(stats['solve_times'])
        stats['median_solve_time'] = np.median(stats['solve_times'])
        stats['min_solve_time'] = np.min(stats['solve_times'])
        stats['max_solve_time'] = np.max(stats['solve_times'])
    
    if stats['speedup_ratios']:
        stats['avg_speedup'] = np.mean(stats['speedup_ratios'])
        stats['median_speedup'] = np.median(stats['speedup_ratios'])
...
def plot_time_distribution(stats, output_dir='supervenn_output'):
    ...
    if stats['solve_times']:
        ...
        plt.close()
        logger.info(f'时间分布图已保存到: {os.path.join(output_dir, "solve_time_distribution.png")}')
    ...
    if stats['speedup_ratios']:
        ...
        plt.close()
        logger.info(f'加速比分布图已保存到: {os.path.join(output_dir, "speedup_distribution.png")}')
    ...
    if stats['time_distribution']:
        ...
        plt.close()
        logger.info(f'时间范围分布图已保存到: {os.path.join(output_dir, "time_range_distribution.png")}')
...
def compare_solvers(result_files, solver_names):
    ...
    for solver_name, stats in all_stats.items():
        ...
        print(f'{solver_name:<15} {stats["total"]:<10} {stats["succeed"]:<10} '
    ...
    plt.close()
    logger.info('求解器比较图已保存到: supervenn_output/solver_comparison.png')
