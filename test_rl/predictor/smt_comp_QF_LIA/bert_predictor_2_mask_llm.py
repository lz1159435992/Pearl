...
def train(model, train_loader, test_loader,criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            no_improve_epochs = 0
            torch.save(model.state_dict(), 'QF_LIA_bert_predictor_2_mask_best_model_llm.pth')
        else:
            no_improve_epochs += 1
        ...
        if no_improve_epochs >= early_stopping_patience:
            ...
            break
        torch.save(model.state_dict(), 'QF_LIA_bert_predictor_2_mask_final_model_llm.pth')
...
def training():
    ...
    model = EnhancedEightClassModelLargeInput()
    ...
    # 训练模型
    num_epochs = 400
    train(model, train_loader, test_loader, criterion, optimizer, scheduler, num_epochs)

    # 加载最佳模型
    model.load_state_dict(torch.load('QF_LIA_bert_predictor_2_mask_best_model_llm.pth'))
