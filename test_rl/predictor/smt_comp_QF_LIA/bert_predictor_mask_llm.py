...
def train():
    with open('QF_LIA_train.json', 'r') as file:
    ...
    # 保存最终模型
    torch.save(model.state_dict(), 'QF_LIA_bert_predictor_mask_final_llm.pth')
...
def test():
    ...
    # 你可以根据需要加载最佳模型或最终模型
    if not os.path.exists('QF_LIA_bert_predictor_mask_best_llm.pth'):
        model_path = 'QF_LIA_bert_predictor_mask_final_llm.pth'
    else:
        model_path = 'QF_LIA_bert_predictor_mask_best_llm.pth'
    state_dict = torch.load(model_path)

    # 步骤4: 将状态字典应用到模型
    model.load_state_dict(state_dict)

    with open('QF_LIA_test.json', 'r') as file:
...
def spilt_files():
    import random
    with open('/home/lz/PycharmProjects/Pearl/test_rl/predictor/smt_comp_QF_LIA/embeding_QF_LIA.json', 'r') as file:
    ...
    with open('QF_LIA_train.json', 'w') as file:
        json.dump(train_set, file, indent=4)
    with open('QF_LIA_test.json', 'w') as file:
