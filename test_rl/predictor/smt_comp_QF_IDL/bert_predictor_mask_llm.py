...
def train():
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_loss < best_val_loss:
            ...
            no_improve_epochs = 0
            torch.save(model.state_dict(), 'QF_IDL_bert_predictor_mask_best_llm.pth')
        else:
            no_improve_epochs += 1
    ...
    # 保存最终模型
    torch.save(model.state_dict(), 'QF_IDL_bert_predictor_mask_final_llm.pth')
...
def test():
    ...
    # 你可以根据需要加载最佳模型或最终模型
    if not os.path.exists('QF_IDL_bert_predictor_mask_best_llm.pth'):
        model_path = 'QF_IDL_bert_predictor_mask_final_llm.pth'
    else:
        model_path = 'QF_IDL_bert_predictor_mask_best_llm.pth'
