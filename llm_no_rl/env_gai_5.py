...
class ConstraintSimplificationEnv_test(Environment):
    ...
    def __init__(self, embedder, z3ast, model, model_time, smtlib_str, file_path, var_dict, constant_list):
        ...
        #添加一些常用的取值
        self.constant_list.extend(['0', '1', '-1', '-2147483647','-2147483648','-2147483649','2147483647','2147483648','2147483649'])
        self.step_count = 0
        ...
        #初始化存储范围,在上面初始化了
        # self.var_range_dict = {}
        self.var_range_dict_n = {var_name: [] for var_name in self.var_range_dict.keys()}
        self.range_init()
        # 直接使用字典字面量来初始化
        self.time_dict = {
    ...
    def range_init(self):
        for variable in self.variables:
            ...
            interval_length = (max_value - min_value) / 1000
            intervals = []
            for i in range(10000):
                # 计算每个子区间的上下界
                sub_start = min_value + i * interval_length
                sub_end = min_value + (i + 1) * interval_length

                # 最小值取整，最大值进位
                rounded_start = math.floor(sub_start)
                rounded_end = math.ceil(sub_end)

                intervals.append([rounded_start, rounded_end])

            self.var_range_dict_n[variable] = intervals
            # #添加新的约束
    ...
    def reset(self, seed=None):
        ...
        self.actions = get_actions(
                                   torch.arange(0, len(self.variables)), torch.arange(0, 1001)
                                   )
    ...
    def handle_unknown(self,solver, time_out, reward, performance, smtlib_str):
        ...
        var_list = normalize_smt_str_without_replace(smtlib_str)
        new_state = self.embedder.get_max_pooling_embedding(smtlib_str, var_list)
        ...
        if predicted_solvability == 1:
            ...
            r = solver.check()
            return self.handle_case(r, solver, time_out, reward, performance)
    ...
    def handle_case(self, r, solver, time_out, reward, performance):
        if r == z3.sat:
            ...
            return self.handle_unknown(solver, time_out, reward, self, self.smtlib_str)
        else:
            return self.handle_unsatisfiable(time_out, reward)
    def handle_unsatisfiable(self, time_out, reward):
    ...
    def step(self, action):
        ...
        if self.action_count < 5:
            ...
            try:
                ...
                action_v1 = self.action_list[0][0]
                action_n1 = self.action_list[0][1]
                action_v2 = self.action_list[1][0]
                action_n2 = self.action_list[1][1]
                action_v3 = self.action_list[2][0]
                action_n3 = self.action_list[2][1]
                action_v4 = self.action_list[3][0]
                action_n4 = self.action_list[3][1]
                action_v5 = self.action_list[4][0]
                action_n5 = self.action_list[4][1]

                numbers = [action_v1, action_v2, action_v3, action_v4, action_v5]
                var_count = 0
                for i in range(len(numbers)):
                    for j in range(i + 1, len(numbers)):
                        if numbers[i] == numbers[j]:
                            var_count += 1
                var_count = var_count // 2  # 每对数字被计算了两次，所以需要除以2
                if var_count > 0:
                    reward -= var_count * 5
                #处理第一个动作
                alpha = 0.6
                reward += self.process_action(action_v1, action_n1, reward) * alpha
                reward += self.process_action(action_v2, action_n2, reward) * alpha
                reward += self.process_action(action_v3, action_n3, reward) * alpha
                reward += self.process_action(action_v4, action_n4, reward) * alpha
                reward += self.process_action(action_v5, action_n5, reward) * alpha
...
def get_actions(tensor_1d_1, tensor_1d_2):
    ...
    # 确保结果张量在正确的设备上
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    result_tensor = cartesian_product_1_2.to(device)
